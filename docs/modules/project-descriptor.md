...
standard `$schema` property shown above. Pin the schema commit so editor caches
cannot retain a different schema under the same URL. Editors can then validate
the file and offer completion directly from the public schema. The property is
authoring metadata and is omitted from the normalized descriptor.
